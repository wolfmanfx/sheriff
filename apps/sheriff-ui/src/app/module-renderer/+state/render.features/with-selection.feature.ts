  ...
  signalStoreFeature(
    ...
    withMethods((store, http = inject(HttpClient), app = inject(AppStore)) => ({
      computeTargets(sel: string) {
        // Skip API call if using external target modules
        if (store.useExternalTargetModules()) return;
      ...
      toggleSelected(id: string) {
        ...
        else if (!store.useExternalTargetModules()) patchState(store, { targetModules: new Set<string>() });
      },
